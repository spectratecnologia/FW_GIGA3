...
void initLCD(){
	LCD_ConfigurePort(GPIOE,
					...
					GPIO_Pin_4, GPIO_Pin_5, GPIO_Pin_6, GPIO_Pin_7, GPIO_Pin_11
					);
...
void LCD_TurnBacklightOn(void){
	...
	gpio_InitTypeDef.GPIO_Pin =  LCD_BACKLIGHT;
	gpio_InitTypeDef.GPIO_Speed = GPIO_Speed_50MHz;
	gpio_InitTypeDef.GPIO_Mode = GPIO_Mode_Out_PP;
	GPIO_Init(LCD_Port, &gpio_InitTypeDef);

	GPIO_WriteBit(LCD_Port, LCD_BACKLIGHT, Bit_SET);
...
void LCD_TurnBacklightOff(void){
	...
	GPIO_WriteBit(LCD_Port, LCD_BACKLIGHT, Bit_RESET);
}
