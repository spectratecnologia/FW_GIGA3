...
static int idioma=SPANISH;

bool isUSBHostMode=false;
uint8_t usbSaveMode=0;

bool msgTimoutReached (uint32_t timeout);
...
Transition smTrans[] =  		//TABELA DE ESTADOS
{
...
void LCD_printLine(uint8_t line, char *string){
	if(line==0){
...
void LCD_vMainScreen(void)
{
	...
	{
		snprintf(lines[1],LINE_SIZE,"     %02d%c%02d      ",(int)(currentTime.currentHour),timerSeparationChar,(int)(currentTime.currentMinute));
		LCD_printLine(0,lines[0]);
	...
	{
		snprintf(lines_spanish[1],LINE_SIZE,"     %02d%c%02d      ",(int)(currentTime.currentHour),timerSeparationChar,(int)(currentTime.currentMinute));
...
//	time_t rawtime;
	...
	snprintf(lines[1],LINE_SIZE,"%02d/%02d/%04d %02d:%02d ", (int)(currentTime.currentDay), (int)(currentTime.currentMonth), (int)(currentTime.currentYear),
															 (int)(currentTime.currentHour), (int)(currentTime.currentMinute));
...
void LCD_vMenuDebug(void){
	static char lines[][LINE_SIZE]={"      Menu      ",
											"Diag. Manual    ",
											"Master/Slave    ",
											"Entradas        ",
											"CAN             ",
...
//	uint32_t *id = getUniqueID();	// function not found when porting to Atollic... commented
...
void LCD_vSetProgressBar (uint8_t progress)
{
	...
	static char progressBar[LINE_SIZE] = "****************";
	static uint8_t percentages[] = {1,2,3,5,7,8,10,12,14,15,15};
	char *progressStart;
