...
Transition MpxSMTrans[] =  		//TABELA DE ESTADOS
{
...
void mpxTest_vStateMachineInit(void)
{
	...
	MpxStateMachine.trans = &MpxSMTrans[0];
...
void mpxTest_vExecute_Flash(void)
{
	...
	if (mpx.ackIndex == 0)
	{
		...
		sendCanPacket(CAN1, CAN_COMMAND_WRITE, CAN_INDEX_EME_IGN_START, MY_ID, mpx.mpxId, (uint8_t*)(&mpx.portEmeIgn[0]), 8);
	}
	...
	else if ((mpx.ackIndex >= CAN_INDEX_EME_IGN_START) && (mpx.ackIndex < (CAN_INDEX_EME_IGN_START + NUM_EFFECTIVE_PORTS)))
	{
		if(mpx.ackIndex%2)
		{
			...
			sendCanPacket(CAN1, CAN_COMMAND_WRITE, mpx.ackIndex + 1, MY_ID, mpx.mpxId, (uint8_t*)(&mpx.portEmeIgn[mpx.ackIndex + 1 - CAN_INDEX_EME_IGN_START]), 8);
		}
	...
	else if (mpx.ackIndex == (CAN_INDEX_EME_IGN_START + NUM_EFFECTIVE_PORTS))
	{
		...
		sendCanPacket(CAN1, CAN_COMMAND_WRITE, CAN_INDEX_EME_N_IGN_START, MY_ID, mpx.mpxId, (uint8_t*)(&mpx.portEmeNIgn[0]), 8);
	}
	...
	else if ((mpx.ackIndex >= CAN_INDEX_EME_N_IGN_START) && (mpx.ackIndex < (CAN_INDEX_EME_N_IGN_START + NUM_EFFECTIVE_PORTS)))
	{
		if(mpx.ackIndex%2)
		{
			...
			sendCanPacket(CAN1, CAN_COMMAND_WRITE, mpx.ackIndex + 1, MY_ID, mpx.mpxId, (uint8_t*)(&mpx.portEmeNIgn[mpx.ackIndex + 1 - CAN_INDEX_EME_N_IGN_START]), 8);
		}
